...
class GCN4ConvSigmoid(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        super(GCN4ConvSigmoid, self).__init__(**kwargs)
    ...
    def forward(self, x: Tensor, edge_index: Adj, denser_edge_index: Adj,
        ...
        if self.training:
            ...
            edge_score, edge_label, new_edge, del_edge = self._get_new_edge(x, denser_edge_index, neg_edge_index)
    ...
    def _get_edge_score(self, x_i, x_j) -> torch.Tensor:
        ...
        print('edge_score', edge_score, edge_score.shape) # 26517, 1
        edge_score = self.r_scaling_5 * F.sigmoid(edge_score) + self.r_bias_5
        print('edge_score', edge_score, edge_score.shape) # 26517, 1
    ...
    def get_link_prediction_loss(model):
        ...
        loss_list = []
        cache_list = [(m, m.cache) for m in model.modules() if m.__class__.__name__ == GCN4ConvSigmoid.__name__]
